        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('open.courses.destroy', ['course' => $course->id]) }}" method="POST">
                    @method('DELETE')
                    @csrf
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="id" value="{{ old('id', $course->id) }}" type="text" disabled/>
                    </label>
                    ...
                    <label class="block text-sm">
                        ...
                        <input class="bg-gray-200  block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="title" value="{{ old('title', $course->title) }}" type="text" disabled/>
                    </label>
                    ...
                    <label class="block text-sm">
                        ...
                        <textarea
                            ...
                            focus:outline-none focus:shadow-outline" name="description" disabled>{{ old('description', $course->description) }}</textarea>
                    </label>
