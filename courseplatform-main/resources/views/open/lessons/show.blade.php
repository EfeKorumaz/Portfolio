        ...
        <div class="card-body grid grid-cols-1 gap-6 lg:grid-cols-1">
            <div class="p-4">
                <form id="form" class="shadow-md rounded-lg px-8 pt-6 pb-8 mb-4"
                      action="{{ route('admin.modules.update', ['lesson' => $lessons->id]) }}" method="POST">
                    @method('PUT')
                    @csrf
                    <label class="block text-sm">
                        ...
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="id" value="{{ $lessons->id }}" type="text" disabled/>
                    </label>
                    <label class="block text-sm">
                        <span class="text-gray-700">Name</span>
                        <input class="bg-gray-200  block rounded w-full p-2 mt-1 focus:border-purple-400
                        focus:outline-none focus:shadow-outline-purple form-input"
                               name="title" value="{{ $lessons->title }}" type="text" disabled/>
                    </label>
                    <label class="block text-sm">
                        ...
                        <textarea
                            ...
                            id="description" rows="7" disabled>{{ $lessons->description }}</textarea>
                    </label>
                    <
